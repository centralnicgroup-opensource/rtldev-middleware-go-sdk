...
func TestSetSession1(t *testing.T) {
	...
	if strings.Compare(tmp, "s_login=myaccountid&s_sessionid=12345678&s_command=COMMAND%3DStatusAccount") != 0 {
		t.Error("TestSession1: Expected post data string not matching." + tmp)
	}
}
...
func TestSaveAndReuseSession(t *testing.T) {
	...
	// Validate the result
	if strings.Compare(tmp, "s_login=myaccountid&s_sessionid=12345678&s_command=COMMAND%3DStatusAccount") != 0 {
		t.Error("TestSaveReuseSession: Expected post data string not matching." + tmp)
	}
}
